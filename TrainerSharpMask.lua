...
--------------------------------------------------------------------------------
-- function: train
function Trainer:train(epoch, dataloader, file)
  print('| start train')
  self.model:training()
  ...
  for n, sample in dataloader:run() do
    ...
    self.model:backward(self.inputs, gradOutputs, self.labels, self.criterion)
    ...
    if n%10 == 0 then
      ...
      -- write log
      local logepoch = string.format('[train] | epoch %05d | s/batch %04.2f | iter: %05d | loss: %07.5f ',
      epoch, timer:time().real/dataloader:size(),n,self.lossmeter:value())
      print(logepoch)
      self.log:writeString(string.format('%s\n',logepoch))
      self.log:synchronize()

      if n%500 == 0 then
        torch.save(string.format('%s/model_%d.t7', self.rundir, n),
        self.modelsv)
      end
    end
  end
...
function Trainer:test(epoch, dataloader)
  ...
  for n, sample in dataloader:run() do
    ...
    if n%10 == 0 then
      print(n, ' test: ', self.lossmeter:value())
    end
