...
cmd:option('-dm', 'pretrained/deepmask', 'path to trained deepmask (if dm, then train SharpMask)')
cmd:option('-km', 32, 'km')
cmd:option('-ks', 32, 'ks')
...
for i = 1, config.maxepoch do
  trainer:train(epoch,trainLoader, fTrain)
  if i%2 == 0 then 
    print '| testing start'
    trainer:test(epoch,valLoader) end
  epoch = epoch + 1
end
fTrain:close()
