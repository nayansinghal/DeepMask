...
Infer.__init = argcheck{
  ...
  call =
    function(self, np, scales, meanstd, model, iSz, dm, timer)
      ...
      self.refs2 = model.refs2
      self.trunk2 = model.trunk2
      self.neth2s = model.neth2s
...
function Infer:forward(input,id)
  ...
  for i,_ in pairs(inpPyramid) do
    ...
    -- forward trunk
    if self.timer then sys.tic() end
    local outTrunk = inpPad

    for k = 1, #self.trunk2 do
      outTrunk = self.trunk2[k]:forward(outTrunk)
    end
    ...
    if not self.dm then
      ...
      for k,neth in pairs(self.neths) do
        if self.timer then sys.tic() end
        neth:forward(self.trunk2[self.skpos[k]].output)
        cutorch.synchronize()
...
function Infer:forwardRefinement(input)
  ...
  local currentOutput = self.refs2[1]:forward(input[1])

  for i = 2,#self.refs2 do
    currentOutput = self.refs2[i]:forward({input[i],currentOutput})
  end
  cutorch.synchronize()
